...
import { Eye, EyeOff, TrendingUp, Shield, Building2, ArrowRight, Loader2, User, Mail, Lock, Briefcase } from 'lucide-react'
...
export default function SignupPage() {
  ...
  const handleSuccessAuth = async (user: any, name: string) => {
    ...
    if (user.providerData.some((p: any) => p.providerId === 'google.com')) {
      toast.success('Google sign up successful! Please set a password for future use.')
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      await handleSuccessAuth(result.user, result.user.displayName || '')
    } catch (err: any) {
  ...
  async function handleVerifyOtp(e: React.FormEvent) {
    ...
    try {
      ...
      if (authRes.user) {
        await updateProfile(authRes.user, { displayName: fullName })
      }
      await handleSuccessAuth(authRes.user, fullName)
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      {/* Body - Direction Swapped: Form on Left, Hero on Right */}
      <div className="flex flex-1 flex-col-reverse lg:flex-row-reverse">
        
        {/* Right — value prop */}
        <div className="hidden lg:flex flex-col justify-between w-[55%] p-14 border-l border-border bg-muted/10">
          {/* Logo */}
          ...
          <div className="space-y-10">
            <div className="space-y-5">
              <div className="inline-flex items-center gap-2 bg-emerald-500/20 text-emerald-500 text-[11px] font-medium px-3 py-1 rounded-full border border-emerald-500/20">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
                Join 12,000+ Verified Real Estate Investors
              </div>
              <h1 className="text-[2.6rem] font-bold text-foreground leading-[1.15] text-balance">
                Create Your Account &<br />Start Fractional Investing
              </h1>
              <p className="text-muted-foreground text-[15px] leading-relaxed max-w-md">
                Get instant access to vetted Grade-A commercial properties, monthly rental yields, and seamless liquidity.
              </p>
            </div>
        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="w-full max-w-[380px] space-y-6">
            ...
            <div>
              <h2 className="text-foreground text-xl font-semibold">Create Account</h2>
              <p className="text-muted-foreground text-sm mt-1">Get started with your free account</p>
            </div>

            {step === 'details' ? (
              <form onSubmit={handleSendOtp} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-muted-foreground text-[11px] font-medium uppercase tracking-wider">Full Name *</label>
                  <Input
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-muted-foreground text-[11px] font-medium uppercase tracking-wider">Confirm *</label>
                    <Input
                      type={showPw ? 'text' : 'password'}
                      placeholder="••••••••"
                ...
                <Button
                  ...
                  className="w-full h-11 bg-primary hover:bg-blue-600 text-primary-foreground font-semibold gap-2 transition-colors mt-2"
                >
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-muted-foreground text-[11px] font-medium uppercase tracking-wider">Verification Code *</label>
                  ...
                  <Input
                    ...
                    required
                    maxLength={6}
                    className="h-11 bg-muted border-border text-foreground placeholder:text-muted-foreground focus-visible:border-primary focus-visible:ring-1 focus-visible:ring-primary/30 tracking-widest text-center"
                  />
                </div>
                {error && (
                  <div className="rounded bg-loss/10 border border-loss/25 text-loss text-xs px-3 py-2.5">
